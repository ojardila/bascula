...
docker run -d --rm --name "$NAME" \
  -e POSTGRES_USER=postgres -e POSTGRES_PASSWORD=postgres -e POSTGRES_DB=bascula \
  -e TZ=UTC -e PGTZ=UTC \
...
ADMIN_DATABASE_URL="postgres://postgres:postgres@127.0.0.1:$PORT/bascula?sslmode=disable" \
