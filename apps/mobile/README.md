...
The Expo app: what happens at the plot and at the payment table. Works fully
offline; the server in [`services/api`](../../services/api) is not needed.
...
Run these from the repository root, which owns the workspaces:

```bash
npm install
npm run android       # or: npm run ios, npm run mobile
npm test              # 75 tests, no build step and no test dependency
```
